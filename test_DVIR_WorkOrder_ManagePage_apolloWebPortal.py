...
def DVIR_WorkOrder_ManagePage(driver, driver_Name, driver_Version, carrier, truckDriversList, driver_UserName_Var,
    ...
    sort_Timestamp_DVIRReport_DVIRPageAdr = "/html/body/app-root/app-main/div/div/app-dvir/section[2]/div/div/div/div[2]/dx-data-grid/div/div[5]/div[2]/table/tbody/tr/td[2]/div[1]"
    ...
    a = w.until(EC.presence_of_element_located((By.XPATH, header_Timestamp_DVIRReport_DVIRPageAdr))).click()
    time.sleep(2)
    b = w.until(EC.element_to_be_clickable((By.XPATH, sort_Timestamp_DVIRReport_DVIRPageAdr))).click()
    ...
    #   Defect List
    #   Defect types
    defectList_ManageWorkOrder_DVIRManageWorkOrderChildPage = w.until(
        EC.presence_of_all_elements_located((By.XPATH, defectList_ManageWorkOrder_DVIRManageWorkOrderChildPageAdr)))
    ...
    try:
        plusNewbutton = driver.find_element(By.XPATH,
                                            button_PlusNew_ManageWorkOrder_DVIRManageWorkOrderChildPageListAdr)
        actions = ActionChains(driver)
        actions.move_to_element(plusNewbutton).perform()
        w.until(EC.element_to_be_clickable(
            (By.XPATH, button_PlusNew_ManageWorkOrder_DVIRManageWorkOrderChildPageListAdr))).click()
        #   Work Order PAGE / Manage Work Order
        #       Title of the Work Order - Manage Work Order Child page
        title_Work_Order_ManageWorkOrder_DVIRManageWorkOrderChildPage = w.until(EC.presence_of_element_located(
            (By.XPATH, title_Work_Order_ManageWorkOrder_DVIRManageWorkOrderChildPageAdr))).text
        ...
        for x in defectList_ManageWorkOrder_DVIRManageWorkOrderChildPageList:
            if x[0] != "(":
                ...
                input_CreatedBy_Work_Order_ManageWorkOrder_DVIRManageWorkOrderChildPageRead = input_CreatedBy_Work_Order_ManageWorkOrder_DVIRManageWorkOrderChildPage.get_attribute(
                    'value')
                ...
                #       SAVE button
                w.until(EC.element_to_be_clickable(
                    (By.XPATH, button_SAVE_Work_Order_ManageWorkOrder_DVIRManageWorkOrderChildPageAdr))).click()
                time.sleep(4)
                try:
                    plusNewbutton = driver.find_element(By.XPATH,
                                                        button_PlusNew_ManageWorkOrder_DVIRManageWorkOrderChildPageListAdr)
        ...
        w.until((EC.element_to_be_clickable((By.XPATH, button_Action_ManageWorkOrder_DVIRPageAdr)))).click()
        newDefectList_ManageWorkOrder_DVIRManageWorkOrderChildPage = w.until(
            EC.presence_of_all_elements_located((By.XPATH, defectList_ManageWorkOrder_DVIRManageWorkOrderChildPageAdr)))

        # Check the WORK ORDER # on the Manage Work Order - Defect List
        n = 0

        print(newDefectList_ManageWorkOrder_DVIRManageWorkOrderChildPage[0].text)
        print(defectList_ManageWorkOrder_DVIRManageWorkOrderChildPage.text)
        try:
        ...
        w.until(EC.presence_of_element_located(
            (By.XPATH, button_X_ManageWorkOrder_DVIRManageWorkOrderChildPageListAdr))).click()
