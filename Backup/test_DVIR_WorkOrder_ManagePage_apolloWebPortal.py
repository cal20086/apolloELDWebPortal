...
def DVIR_WorkOrder_ManagePage (driver, driver_Name, driver_Version, carrier, truckDriversList, driver_UserName_Var, client_Name_Var, client_Adress_Var, defect_Vehicle_List_Var, workOrder_CreatedBy_Var, workOrder_Contacts_Var):
...
#   Verify DVIR Report - Driver is corrected: ??????????????????????????????????????????????????????????????????????????????????????????????????
    driver_DVIRReport_DVIRPagetable = w.until(EC.presence_of_element_located((By.XPATH, "/html/body/app-root/app-main/div/div[1]/app-dvir/section[2]/div/div/div/div[2]/dx-data-grid/div/div[6]/div[1]/div/div[1]/div/table/tbody/tr[1]/td[2]")))
...
#   Client Name
    clientName_ManageWorkOrder_DVIRManageWorkOrderChildPageRead = w.until(EC.presence_of_element_located((By.XPATH, clientName_ManageWorkOrder_DVIRManageWorkOrderChildPageAdr))).text
...
#   +New button
    try:
        a = w.until(EC.element_to_be_selected((By.XPATH, button_PlusNew_ManageWorkOrder_DVIRManageWorkOrderChildPageListAdr))).click()
    ...
    #   Defect List
        ...
        for x in defectList_ManageWorkOrder_DVIRManageWorkOrderChildPageList:
            if x[0] != "(":
                ...
                input_DefectList_Work_Order_ManageWorkOrder_DVIRManageWorkOrderChildPage.send_keys(defectList_ManageWorkOrder_DVIRManageWorkOrderChildPageList[n])
